...
export default function NewJobListingPage() {
  ...
  const validateFile = (file) => {
    ...
    if (!allowedTypes.includes(file.type) && !allowedExtensions.includes(fileExtension)) {
      message.error(`${file.name} has an invalid file type. Allowed types: PDF, DOC, DOCX, XLS, XLSX, TXT, JPG, PNG`);
      return false;
    }
  ...
  const handleUpload = (setter, field) => async (file) => {
    // D92: Validate file before upload
      ...
      content: (
        <Space direction="vertical" style={{ width: '100%' }} size="large">
          ...
          <Space wrap>
            ...
            <Form.Item 
              name="projectEnd" 
              label="Project end (optional)"
              rules={[
                {
                  validator: (_, value) => {
                    if (!value) return Promise.resolve();
                    const endDate = value.startOf('month');
                    const projectStart = form.getFieldValue('projectStart');
                    // D95: Validate end date is after start date
                    if (projectStart) {
