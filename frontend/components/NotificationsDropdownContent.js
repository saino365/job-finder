...
export default function NotificationsDropdownContent({
  ...
  // Only show unread notifications in the dropdown (limit to 10 most recent unread)
  const unreadNotifs = safeNotifs.filter(n => !n.isRead).slice(0, 10);
  
  const directNotifs = unreadNotifs.filter(n => {
    const channel = n.channel || '';
    const type = n.type || '';
    return channel !== 'watching' && type !== 'watching' && !type.includes('job');
  });
  
  const unreadTag = (n) => (!n.isRead ? <Tag color="blue">Unread</Tag> : null);
  const statusTag = (n) => {
